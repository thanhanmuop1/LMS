...
const CreateQuizBase = ({ role, apiEndpoint }) => {
    ...
    if (role === 'teacher') {
      const fetchCourses = async () => {
        try {
          ...
          const response = await axiosConfig.get(`/teacher/courses`, {
  ...
  const fetchQuizData = async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await axiosConfig.get(`${apiEndpoint}/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (isEditing) {
        ...
        await axiosConfig.put(`/quizzes/${id}`, {
          ...quizData,
          questions
        }, {
          headers: { 'Authorization': `Bearer ${token}` }
        ...
        await axiosConfig.post(`/quizzes`, {
          ...quizData,
          questions: []
        }, {
          headers: { 'Authorization': `Bearer ${token}` }
