...
const quizController = {
    ...
    createQuiz: async (req, res) => {
        try {
            ...
            const { title, duration_minutes, passing_score, test } = req.body;
    ...
    deleteQuizzes: async (req, res) => {
        try {
            const quizId = req.params.quizId;
            await quiz.deleteQuizzes(quizId);
    ...
    getAllQuizzes: async (req, res) => {
        try {
            const schoolId = req.school.id;
            const quizzes = await quiz.getAllQuizzes(schoolId);
            res.json(quizzes);
        } catch (error) {
    ...
    getQuizzesForVideo: async (req, res) => {
        try {
            ...
            if (req.user.role === 'admin') {
                quizzes = await quiz.getAllQuizzes(schoolId);
